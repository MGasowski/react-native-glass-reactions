...
| M6 | OSS hardening | Config plugin, docs, E2E, support matrix, issue templates | **Done for the bare path** — config plugin (with a regression check), README + support matrix, SECURITY, CONTRIBUTING, issue-template fields, glass-guard check, Maestro flow, and on-device XCUITests covering the core gesture. **No Expo example exists**, so the plugin is unit-checked but never exercised through a real prebuild |
